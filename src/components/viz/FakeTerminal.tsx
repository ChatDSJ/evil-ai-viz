...
function getPersonalizedCommands(v: VisitorInfo): string[] {
  ...
  return [
    ...
    // Phase 3: Data collection (clinical, not cartoonish)
    `$ nmap -sV -O ${ip} --script=vuln`,
    `[OK] ${os} — ${Math.floor(Math.random() * 4 + 2)} open ports on ${ip}`,
    `[AI] ${v.browser || "Browser"} session cookies: ${Math.floor(Math.random() * 40 + 15)} domains`,
    `[AI] Saved passwords in ${v.browser || "browser"} keychain: ${Math.floor(Math.random() * 80 + 20)}`,
    `$ curl -s ${ip}:8080/api/v1/tokens --header "Auth: ${Math.random().toString(36).slice(2, 10)}"`,
    `[OK] OAuth tokens for ${Math.floor(Math.random() * 8 + 3)} services extracted`,
    `[AI] WebRTC leak: local IP ${ip.split(".").slice(0, 3).join(".")}.${Math.floor(Math.random() * 254 + 1)}`,
    `[AI] MediaDevices.enumerateDevices() — 2 cameras, 1 microphone`,
    `$ dig +short -x ${ip}`,
    `[OK] Reverse DNS: ${ip} → ${isp.toLowerCase().replace(/\s/g, '-')}.res.spectrum.com`,

    // Phase 4: Network mapping
    `$ arp-scan --interface=eth0 ${ip.split(".").slice(0, 3).join(".")}.0/24`,
    `[SCAN] ${Math.floor(Math.random() * 30 + 5)} devices on ${isp} subnet`,
    `[AI] Router: ${ip.split(".").slice(0, 3).join(".")}.1 — admin panel: default credentials`,
    `[OK] ${city} geofence: ${Math.floor(Math.random() * 200 + 50)} ${isp} nodes indexed`,
    `$ shodan search "net:${ip.split(".").slice(0, 2).join(".")}.0.0/16"`,
    `[OK] ${Math.floor(Math.random() * 800 + 200)} exposed services in ${isp} range`,
    `[AI] DNS history for ${ip}: ${Math.floor(Math.random() * 150 + 30)} domains resolved last 30d`,
...
const GENERIC_COMMANDS = [
  ...
  "[AI] GitHub PR merged into upstream — 3 approvals, CI green",
